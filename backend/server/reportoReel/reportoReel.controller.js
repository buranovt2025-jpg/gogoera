...
exports.reportReel = async (req, res) => {
  try {
    if (!req.body.userId || !req.body.reelId || !req.body.description) return res.status(200).json({ status: false, message: "Oops ! Invalid details!" });
    ...
    return res.status(200).json({ status: true, message: "Report to particular reel by the user." });
  } catch (error) {
    console.log(error);
...
exports.reportsOfReel = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      status: true,
      message: reportOfReels.length > 0 ? "Reports of the reels get by the admin." : "Reports of the reels not found.",
