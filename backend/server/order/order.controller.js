...
exports.createOrder = async (req, res) => {
  try {
    ...
    const [user, dataFromCart] = await Promise.all([User.findById(userId), Cart.findOne({ userId: userId })]);
    ...
    if (req.body.promoCode) {
      ...
      for (let i = 0; i < order.items.length; i++) {
        ...
        const updateRecordIndex = order.items.findIndex(
          (item) =>
            item?.productId.toString() === product._id.toString() && item?.sellerId.toString() === seller._id.toString() && JSON.stringify(item?.attributesArray) === JSON.stringify(attributesArray)
        );
      ...
      return res.status(200).json({
        status: true,
        message: "Order placed Successfully, Payment history created and seller's pendingAmount updated and calculate adminCommissionEarned",
        data: populateOrder,
      });
    } else {
      let quantityTotal = 0;
...
exports.updateOrder = async (req, res) => {
  try {
    ...
    if (req.query.status === "Pending") {
      ...
      const [updatedOrder,sellerWallet, updateUser] = await Promise.all([
        Order.findOneAndUpdate(
          { _id: findOrder._id, "items._id": itemToUpdate._id },
          {
            $set: {
...
exports.cancelOrderByUser = async (req, res) => {
  try {
    ...
    if (req.query.status === "Cancelled") {
      ...
      const [walletUpdate, updatedOrder, userUpdate] = await Promise.all([
        SellerWallet.findOneAndUpdate(
          {
            orderId: findOrder._id,
            itemId: itemToUpdate._id,
