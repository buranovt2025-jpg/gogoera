...
exports.sellerPendingWithdrawableAmount = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({
      status: true,
      message: "Retrive sellerPendingWithdrawableAmount list for the seller.",
...
exports.sellerPendingWithdrawalRequestedAmount = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({
      status: true,
      message: "Retrive sellerPendingWithdrawalRequestedAmount list for the seller.",
...
exports.sellerEarningAmount = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({
      status: true,
      message: "Retrive sellerEarningAmount list for the seller.",
...
exports.getAllAmount = async (req, res) => {
  try {
    ...
    const [seller, pendingAmount, pendingWithdrawbleAmount, pendingWithdrawbleRequestedAmount, earningAmount, totalCommissionGiven] = await Promise.all([
      Seller.findById(sellerId),
      SellerWallet.aggregate([
        {
          $match: {
    ...
    return res.status(200).json({
      status: true,
      message: "Retrive seller's pendingAmount, pendingWithdrawableAmount, pendingWithdrawbleRequestedAmount, earningAmount and totalCommissionGiven.",
      ...
      pendingWithdrawbleRequestedAmount: pendingWithdrawbleRequestedAmount.length > 0 ? pendingWithdrawbleRequestedAmount[0].pendingWithdrawbleRequestedAmount : 0,
...
exports.sellerTransaction = async (req, res) => {
  try {
    ...
    if (transactions.length === 0) {
      ...
      return res.status(200).json({
        status: true,
        message: "Retrive transactions for the seller",
        transactions: data,
      });
    }
  } catch (error) {
...
exports.sellerPendingWithdrawalRequestedAmountForAdmin = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({
      status: true,
      message: "Retrive sellerPendingWithdrawalRequestedAmount list for admin!",
...
exports.byAdminToSeller = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({
      status: true,
      message: "seller's pendingWithdrawalRequestedAmount given by admin to the seller.",
...
exports.adminCommissionWallet = async (req, res) => {
  try {
    ...
    const [totalCommission, totalEarningWithCommission, totalEarningWithoutCommission, todayCommission, monthlyCommission, yearlyCommission] = await Promise.all([
      SellerWallet.aggregate([
        {
          $group: {
            _id: null,
...
exports.chartAnalyticOfRevenue = async (req, res) => {
  try {
    ...
    return res.status(200).json({ status: true, message: "Success", totalCommission, totalEarningWithCommission, totalEarningWithoutCommission });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ status: false, message: error.message || "Internal Server Error" });
  }
