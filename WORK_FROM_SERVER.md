...
**Почему один раз нужны команды с ПК?**  
Правки в Cursor делаются в папке проекта на твоём компьютере. Чтобы они попали на сервер: один раз с ПК делаешь `git add` → `commit` → `push` в GitHub. Дальше всё — только на сервере: `git pull`, деплой, сид. Доступа по SSH у бота нет — он не подключается к твоему серверу.
