...
**Веб без своего ПК (всё на сервере):**  
Скопируй на сервер файл `server_web_setup.sh` из репозитория, подключись по SSH и выполни:
```bash
ssh root@146.190.238.186
# Загрузить скрипт (или скопировать содержимое server_web_setup.sh)
bash server_web_setup.sh
```
Скрипт установит Flutter, соберёт веб из gogoera и настроит Nginx. Открой в браузере: **http://146.190.238.186/** — не зависит от твоего компьютера.
